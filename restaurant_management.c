...
int login()
{
    ...
    if(strcmp(username,"rishabh")==0){
        if(strcmp(password,"Rishu.123")==0){
    ...
    {
        if(strcmp(username,"nachiketa")==0){
        if(strcmp(password,"Nachi.123")==0){
 
 	    printf("login succssfull");
        return 1;
        }
    }
        else{

        printf("Invlid credentials,Try again\n");
        printf("\n");
        login();
        }
    }
    ...
    while(1)
    {
        ...
        switch (opt)
        {
            case 1:
                ...
                //char more;
                //printf("Do you want to order more? (1/0) \n");
                //scanf("%c",&more);
                //if(more==1)
                //{
                   // displayList(heada);
...
int main()
{
    ...
    heada=  createadmin(heada,6,"Dope Margheritta",470);
    heada=  createadmin(heada,7,"White Sauce Pasta",200);
    heada=  createadmin(heada,8,"Nutella Shake",180);
